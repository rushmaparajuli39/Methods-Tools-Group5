...
class Account:
    ...
    def EditShiipingInfo(UserId):
        ...
        print("\nNew Shipping Info: ")
        ship=str(input())
        cursor.execute('UPDATE account SET Shipping_Address = %s AND Shipping_Zip = %s WHERE UserId=%s', (ship, UserId))
        mydb.commit() # # we commit(save) the records to the table 
    ...
    def EditPaymentInfo(UserId):
        ...
        print("\nNew Payment Info: ")
        pay=str(input())
        cursor.execute('UPDATE account SET Billing_Address = %s, Zip = %s, City = %s, State = %s, Card_Name = %s AND Card_Number=%s WHERE UserId=%s', (pay,UserId))
        mydb.commit() # # we commit(save) the records to the table
 ...
 #after login has succeeded
    while(loggedin==True):
        ...
        if(choice==4):
            ...
            if (1):
                print("Enter the ISBN, Price and Title for books.")
                ISBN=input()
                Price=input()
                Title=input()
            ...
            #add to the database for available movies
                print("Enter the Name, Type and Price for movies.")
                Name=input()
                Type=input()
                Price=input()
